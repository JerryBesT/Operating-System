...
fetchint(struct proc *p, uint addr, int *ip)
{
  if(addr < 0x4000) {
	...
	if((p->share_index == 0) && (addr < 0x4000))
		return -1;
	if(p->share_index != 3 && (addr >= 0x1000 * (p->share_index + 1) && addr < 0x4000))
		return -1;
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if(addr >= USERTOP)
    return -1;
  if(addr >= p->sz && addr < p->bottom_stack)
  	return -1;
  *pp = (char*)addr;
  ep = (char*)USERTOP;
  for(s = *pp; s < ep; s++)
...
argptr(int n, char **pp, int size)
{
  ...
  if((uint)i < 0x4000) {
	...
	if((proc->share_index == 0) && ((uint)i < 0x4000))
		return -1;
	if((proc->share_index != 3) && ((uint)i >= 0x1000 * (proc->share_index + 1) && (uint)i < 0x4000))
		return -1;
