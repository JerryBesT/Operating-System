...
void
shareMapping(pde_t *pgdir, void *la, uint size, uint pa, int perm)
{
	mappages(pgdir, la, size, pa, perm);
	return;
}
...
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz, uint st_b, uint sh_top)
{
  pde_t *d;
  pte_t *pte;
  uint pa;
  ...
  // copying the share segment
	for(; share < sh_top + 0x1000;share += PGSIZE) {
