  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
	  ...
	  if(currPrior == 0) {
		  ...
		  int remain = p->ticks[currPrior] % time_slice[currPrior];
		  if(p->ticks[currPrior] == 0)
		  	remain = time_slice[currPrior];
		  if(remain == 0)
		  	p->priority--;
	  ...
	  cprintf("name = %s, pid = %d, currPrior = %d, ticks = %d \n",
	  			p->name, p->pid, currPrior, p->ticks[currPrior]);
    }
    release(&ptable.lock);
  }
